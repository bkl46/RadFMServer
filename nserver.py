...
def create_radfm_optimized_prompt(bg_text: str, image_paths: List[str]) -> tuple:
    """
    Create a prompt optimized for RadFM's expected input format
    """
    HIGH_RECALL_TARGETS = ["pneumonia", "pneumothorax", "pulmonary_edema", "consolidation", "atelectasis"]
    ROBUST_TARGETS = ["fracture", "pleural_effusion", "cardiomegaly", "emphysema", "fibrosis"]
    DEFAULT_FINDINGS = HIGH_RECALL_TARGETS + ROBUST_TARGETS
    
    # Simple, direct prompt that matches RadFM's training
    prompt = (
        f"Clinical history: {bg_text}\n"
        f"Analyze this chest radiograph for: {', '.join(DEFAULT_FINDINGS)}.\n"
        "Provide findings in this format:\n"
        "Findings: [list findings]\n"
        "Diagnosis: [present/absent for each target]\n"
        "Confidence: [0-100]\n"
    )
    ...
    for i, img_path in enumerate(image_paths):
        image_list.append({
            'img_path': img_path,
            'position': 0  # Images at beginning
        })
...
def test_exact_training_format():
    """
    Try to replicate the exact format from RadFM's training
    Based on the paper, RadFM uses: <image> tokens inserted
    """
    # Minimal prompt that should trigger vision
    minimal_prompt = "<image>What is shown in this chest radiograph?<|endoftext|>"
    
    # Image at position 0 (right before the text starts)
    test_image = [{
        'img_path': '/home/bkl46/M/MedVLM001/data/images/images_normalized/612_IM-2199-2001.dcm.png',
        'position': 0
    }]
    
    text, vision_x = combine_and_preprocess(minimal_prompt, test_image, image_padding_tokens)
    
    print(f"DEBUG: Minimal prompt test")
    print(f"DEBUG: Text: {text[:100]}")
    
    lang_x = text_tokenizer(text, max_length=2048, truncation=True, return_tensors="pt")['input_ids']
    ...
    print(f"DEBUG: Response to minimal prompt: {response}")
...
def chat_completions():
    ...
    try:
        ...
        images = messages[1].get("image")


        print("DEBUG: main text prompt: ", prompt)


        print("DEBUG: number of images: ", len(images))
...
def chvis():
    test_vision_encoder_is_working()
    return {"test": "vision"}
...
def testtf():
    test_exact_training_format()
    return {"test": "extact training format"}
